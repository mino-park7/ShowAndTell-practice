...
from im2txt import configuraion
...
def evaluate_model(sess, model, global_step, summary_writer, summary_op):
    ...
    :param model: Instance of ShowAndTellModel; the model to evaluate.
    :param global_step: Integer; global step of the model checkpoint.
    :param summary_writer: Instance of FileWriter.
    :param summary_op: Op for generating model summaries.
    :return:
    """

    # Log model summaries on a single_batch.
...
def run_once(model, saver, summary_writer, summary_op):
    """
    Evaluate the latest model checkpoint.

    :param model: Instance of ShowAndTellModel; the model to evaluate.
    :param saver: Instance of tf.train.Saver for restoring model Variables.
    :param summary_writer: Instance of FileWriter.
    :param summary_op: Op for generating model summaries.
    ...
    with tf.Session() as sess:
        ...
        tf.logging.info("Succesfully loaded %s at global step = %d.",
...
def run():
    ...
    with g.as_default():
        ...
        model_config = configuraion.ModelConfig()
...
def main(unused_argv):
    assert FLAGS.input_file_pattern, "--input_file is required"
