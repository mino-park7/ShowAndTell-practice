...
and
    ...
    val_image_dir/COCO_val2014_0000000042.jpg
    val_image_dir/COCO_val2014_0000000073.jpg
    ...

The MSCOCO annotations JSON files ar expected to reside in train_captions_file
...
is a serialized SequenceExample proto consisting of precisely on image-caption
...
descending frequency) of word tokens in the training set. Only tokes appearing
at least 4 times are considered; all other words get the "unknown" word id.

NOTE: This script will consume around 100GB of disk space because each image
in the MSCOCO dataset is replicated ~5 times (once per caption) in the output.
This is done for two reasons:
    1. In order to vetter shuffle the training data.
...
def _to_sequence_example(image, decoder, vocab):
    ...
    except tf.errors.InvalidArgumentError as AssertionError :
...
def _process_image_files(thread_index, ranges, name, images, decoder, vocab,
    ...
    :param thread_index: Integer thread identifier within [0, len(ranges)].
    :param ranges: A list of pairs of integers specifying the ranges of the dataset to
        process in parallel.
...
def _process_dataset(name, images, vocab, num_shards):
    ...
    :return:
    """
    # Breaks up each image into a separate entity for each caption.
...
def _create_vocab(captions):
    ...
    word in the files is its corresponding 0-based line number.

    :param captions: A list of lists of strings.
    ...
    # Filter uncommon words and sort by sort by descending count.
...
def main(unused_argv):
    ...
    # Redisrtibute the MSCOCO data as follows:
    #   train_dataset = 100% of mscoco_train_dataset + 85% of mscoco_val_dataset.
    #   val_dataset = 5% of mscoco_val_dataset (for validation during training).
    #   test_dateset = 10% of mscoco_val_dataset (for final evaluation).
