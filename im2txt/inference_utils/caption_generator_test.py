...
class CaptionGeneratorTest(tf.test.TestCase):
    ...
    def _assertExpectedCaptions(self,
        ...
        :param expected_captions: A sequence of pairs (sentence, probability), where
        sentence is a list of integer ids and probability is a float in [0,1].
        :param beam_size: Parameter passed to beam_search().
        :param max_caption_length: Parameter passed to beam_search().
        :param length_normalization_factor: Parameter passed to beam_search().
    ...
    def testMaxLength(self):
        ...
        # Max length = 2.
        # There are no complete sentences, so partial sentences ar returned.
        expected = [([0, 4], 0.4), ([0, 3], 0.3), ([0, 2], 0.2)]
        self._assertExpectedCaptions(expected, max_caption_length=2)

        # Max length = 3.
        # There is a least one complete sentence, so only complete sentences are
        # returned.
